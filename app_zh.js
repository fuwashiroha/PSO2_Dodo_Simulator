...
function renderList(){
  ...
  const list=DATA.filter(a=>(!g||displayCategory(a)===g)&&(!q||(`${displayName(a)} ${a.name} ${displayEffect(a)} ${displayCategory(a)}`).toLowerCase().includes(q)));
...
function addToSlot(slot,code){const a=ability(code);if(!a)return;if(a.special!=='junk'&&state.slots[slot].includes(code))return;if(state.slots[slot].length>=8){toast("最多8槽");return}const fk=familyKey(a);if(fk){const old=state.slots[slot].find(c=>familyKey(ability(c))===fk);if(old){state.slots[slot]=state.slots[slot].filter(c=>c!==old);state.factor[slot]=state.factor[slot].filter(c=>c!==old)}}state.slots[slot].push(code);render()}
...
function renderCandidates(){
  ...
  $("#candidateBody").innerHTML=list.map(a=>{
    ...
    return `<label class="cand ${checked?'sel':''}" title="${esc(jpNote(a))}"><input type="checkbox" data-code="${a.code}" ${checked?'checked':''}><span class="candName">${esc(displayName(a))}</span><span class="base">${a.base}%</span><strong>${fin}%</strong></label>`
...
function renderSummary(){
  ...
  $("#summary").innerHTML=`<div><b>已选：</b> ${n}个 ${n>state.slots['本体'].length?'<span class="warn">扩张</span>':''}</div>${state.sameName?`<div><b>同名装备补正：</b> ×${nm.toFixed(2)}</div>`:''}<div><b>综合成功率：</b> <strong>${total.toFixed(2)}%</strong></div><div class="small">按各能力独立判定计算全部成功的概率</div>`
}
...
function initApp(){loadHash();[...new Set(DATA.map(a=>displayCategory(a)))].sort((a,b)=>a.localeCompare(b,'zh-CN')).forEach(g=>$("#group").insertAdjacentHTML('beforeend',`<option>${esc(g)}</option>`));$("#search").oninput=renderList;$("#group").onchange=renderList;$("#support").onchange=e=>{state.support=+e.target.value;render()};$("#campaign").onchange=e=>{state.campaign=+e.target.value;render()};$("#sameName").onchange=e=>{state.sameName=!!e.target.checked;render()};$("#save").onclick=saveLocal;$("#load").onclick=loadLocal;$("#reset").onclick=()=>{if(confirm('确定要全部清空吗？'))resetAll()};$("#copy").onclick=()=>navigator.clipboard?.writeText(location.href).then(()=>toast('URL已复制')).catch(()=>toast('受浏览器限制，无法复制'));render()}
