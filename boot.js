(function(){
  const texts={
    ja:{title:"PSO2 特殊能力追加シミュレーター V27.9",lang:"言語",save:"保存",load:"読込",copy:"URLコピー",reset:"クリア",
        list:"特殊能力一覧",search:"能力名を検索（ひらがな可）",all:"全分類",material:"本体 / 素材",support:"能力追加成功率",none:"無し",
    ...
    "zh-CN":{title:"PSO2 特殊能力追加模拟器 V27.9",lang:"语言",save:"保存",load:"读取",copy:"复制URL",reset:"清空",
        list:"特殊能力列表",search:"搜索能力名（支持平假名）",all:"全部分类",material:"本体 / 素材",support:"能力追加成功率",none:"无",
        campaign:"奖励期间",same:"本体·使用素材全部同名",tip:"仅当本体与当前使用的全部素材装备名完全相同时生效",addItem:"追加道具",addItemTip:"从下拉列表选择1个特殊能力追加道具，仅将该能力加入追加候选",junkFill:"所有本体和素材用占位能力填充至",junkFillSuffix:"孔",junkFillBtn:"执行",junkClear:"删除全部占位能力",candidate:"追加候选 / 成功率"}
  };
  let lang=window.safeStorageGet("pso2-dodo-language")||"ja";
  ...
  $("#language").addEventListener("change",function(){if(window.prepareLanguageChange && !window.prepareLanguageChange(this.value))return;location.reload();});
  $("#lowRarityWeaponText").textContent=lang==="zh-CN"?"本体为★12以下武器（究极克星适用条件）":"本体は★12以下の武器（アルティメットバスター用）";
  $("#lifeGuidanceText").textContent=lang==="zh-CN"?"生命之引导（+10%）":"生命の導き（+10%）";
  $("#abilityProtectionText").textContent=lang==="zh-CN"?"是否使用特殊能力保护":"特殊能力保護の使用";
  $("#abilityProtectionLabel").title=lang==="zh-CN"?"不影响成功率；使用保护时，只要有能力失败，本次结果就不存入背包。":"成功率は変わりません。保護使用時に1つでも失敗すると結果をバッグに保存しません。";
  [...$("#abilityProtection").options].forEach(o=>{o.textContent=o.value?(lang==="zh-CN"?`特殊能力保护(${o.value}孔以下)`:`特殊能力保護(${o.value}枠以下)`):(lang==="zh-CN"?"不使用":"使用しない");});
