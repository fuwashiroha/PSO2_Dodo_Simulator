...
let state = { slots:Object.fromEntries(SLOTS.map(x=>[x,[]])), factor:Object.fromEntries(SLOTS.map(x=>[x,[]])), support:0, campaign:0, sameName:false, selected:[] };
...
function saveHash(){const mini={s:state.slots,f:state.factor,b:state.support,c:state.campaign,n:!!state.sameName};const raw=encodeURIComponent(btoa(unescape(encodeURIComponent(JSON.stringify(mini)))));history.replaceState(null,"","#"+raw)}
function loadHash(){if(!location.hash)return;try{const o=JSON.parse(decodeURIComponent(escape(atob(decodeURIComponent(location.hash.slice(1))))));if(o.s)state.slots=o.s;if(o.f)state.factor=o.f;if(Number.isFinite(o.b))state.support=o.b;if(Number.isFinite(o.c))state.campaign=o.c;if(typeof o.n==="boolean")state.sameName=o.n}catch(e){}}
function saveLocal(){localStorage.setItem("pso2-dodo-offline-v9",JSON.stringify(state));toast("ローカル保存しました")}
function loadLocal(){try{const o=JSON.parse(localStorage.getItem("pso2-dodo-offline-v9")||localStorage.getItem("pso2-dodo-offline-v8")||localStorage.getItem("pso2-dodo-offline-v7")||localStorage.getItem("pso2-dodo-offline-v6")||localStorage.getItem("pso2-dodo-offline-v5")||localStorage.getItem("pso2-dodo-offline-v4")||localStorage.getItem("pso2-dodo-offline-v3")||localStorage.getItem("pso2-dodo-offline-v2")||localStorage.getItem("pso2-dodo-offline"));if(o){state=o;state.selected=state.selected||[];state.sameName=!!state.sameName;render();toast("読込しました")}}catch(e){}}
function resetAll(){state={slots:Object.fromEntries(SLOTS.map(x=>[x,[]])),factor:Object.fromEntries(SLOTS.map(x=>[x,[]])),support:0,campaign:0,sameName:false,selected:[]};render()}
...
function familyKey(a){
  ...
  if(['soul','soul_standard','astral_soul','guardian_soul','ji_soul','ix_soul'].includes(a.special) || a.group==='ソール')return 'soul';
...
function renderList(){const q=$("#search").value.trim().toLowerCase(),g=$("#group").value;const list=DATA.filter(a=>(!g||a.group===g)&&(!q||(a.name+a.effect).toLowerCase().includes(q)));$("#abilityList").innerHTML=list.map(a=>`<div class="ability" draggable="true" data-code="${a.code}" title="${esc(a.effect)}"><span>${esc(a.name)}</span><small>${esc(a.effect||a.group)}</small></div>`).join('');$$("#abilityList .ability").forEach(el=>{el.addEventListener("dragstart",e=>{e.dataTransfer.effectAllowed="copy";e.dataTransfer.setData("text/plain",JSON.stringify({type:"library",code:el.dataset.code}))});el.addEventListener("dblclick",()=>addToSlot("本体",el.dataset.code))})}
function addToSlot(slot,code){const a=ability(code);if(!a)return;if(a.special!=='junk'&&state.slots[slot].includes(code))return;if(state.slots[slot].length>=8){toast("最大8スロットです");return}const fk=familyKey(a);if(fk){const old=state.slots[slot].find(c=>familyKey(ability(c))===fk);if(old){state.slots[slot]=state.slots[slot].filter(c=>c!==old);state.factor[slot]=state.factor[slot].filter(c=>c!==old)}}state.slots[slot].push(code);render()}
...
function candidates(){const err=validationError();if(err)return {list:[],err};const all=SLOTS.flatMap(s=>state.slots[s]);const counts={};all.forEach(c=>counts[c]=(counts[c]||0)+1);const out=[];DATA.forEach(a=>{const base=rawRate(a,counts,all);if(base>0)out.push({...a,base})});return {list:out.sort((a,b)=>b.base-a.base||a.group.localeCompare(b.group,'ja')||a.name.localeCompare(b.name,'ja')),err:''}}
