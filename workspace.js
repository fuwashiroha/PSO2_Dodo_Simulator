...
window.installWorkspace = function(){
 ...
 function clearHighlights(){$$('.dropHint,.insertActive,.dropSwap,.dropCopy').forEach(x=>x.classList.remove('dropHint','insertActive','dropSwap','dropCopy'))}
 ...
 function targetAt(x,y,ctrl){const el=document.elementFromPoint(x,y);if(!el)return null;const gap=el.closest('.insertTarget');if(gap)return ctrl?null:{mode:'insert',index:+gap.dataset.insert,el:gap};const m=el.closest('.mat');return m?{mode:ctrl?'copy':'swap',index:SLOTS.indexOf(m.dataset.slot),el:m}:null}
 function highlight(target){clearHighlights();if(!target)return;target.el.classList.add(target.mode==='insert'?'insertActive':target.mode==='copy'?'dropCopy':'dropSwap')}
 function setupHeaderDrag(head,index){
  ...
  head.onpointerup=e=>{
   ...
   cancelDrag();if(armed&&target&&tab===ws.activeId)equipmentAction(from,target.index,target.mode);
  };
 ...
 function addUI(){
  ...
  $('#candidateTitle').insertAdjacentHTML('beforebegin',`<div class="panelTitle inventoryTitle">${T('道具背包','アイテムバッグ')} <span id="inventoryCount">0</span></div><div id="inventoryGrid"></div>`);
 ...
 window.Workspace={snapshot,parseWorkspace,equipmentAction,inventoryToSlot,addTab,activate,closeTab,undoLast,checkRun,makeResult,startAffix,copyInventory,deleteInventory,showItem,guardianDialog,closeModal,importFile,showSaves,deleteSave,readArchive,editItemDialog,updateInventoryItem};
};
