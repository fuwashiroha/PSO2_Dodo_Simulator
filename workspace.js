...
window.abilitySearchText=(a,...extra)=>[a.name,a.nameZh||'',...(Array.isArray(a.searchAliases)?a.searchAliases:[]),...extra].join(' ');
...
window.installWorkspace = function(){
 ...
 function editItemDialog(id){
  ...
  const renderRows=()=>{
   ...
   const all=DATA.filter(a=>!query||window.normalizeAbilitySearch(window.abilitySearchText(a)).includes(query)).sort((a,b)=>displayName(a).localeCompare(displayName(b),zh?'zh-CN':'ja'));
 ...
 function addUI(){
  ...
  $('#load').insertAdjacentHTML('afterend',`<button id="guardianChain">${T('守护辉士·魂合成链','ガーディアン合成')}</button><button id="exportWorkspace">${T('导出文件','エクスポート')}</button><button id="importWorkspace">${T('导入文件','インポート')}</button><input id="workspaceFile" type="file" accept=".json,application/json" hidden>`);
