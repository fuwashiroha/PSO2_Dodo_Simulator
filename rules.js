...
function recipeRate(a,counts){
  ...
  const recipe=window.GUARDIAN_RECIPES?.find(r=>r.name===n);
  if(recipe){
    const ok=recipe.parts.every(p=>p.names.filter(x=>countName(counts,x)>=(p.count||1)).length>=(p.choose||1));
    return ok?recipe.rate:0;
  }
...
function rawRate(a,counts,all,isFactor=false,lowRarityWeapon=false){
  ...
  // Receptors do not become transferable merely by marking them as a factor.
